...
Практические примеры интеграции для типичных окружений.

## Браузер — Canvas

Простейший способ симулировать изображение в браузере:
...
## Браузер — OffscreenCanvas (Web Worker)

Обработка изображений вне основного потока для плавного интерфейса:
...
## Node.js — Sharp (матричный путь)

Для моделей на основе матриц используйте встроенную функцию Sharp `recomb` для максимальной производительности:
...
::: warning
`gamma(2.2)` в Sharp — это приближение линеаризации sRGB. Для попиксельно точных результатов, совпадающих с выходом библиотеки, используйте путь через сырой буфер ниже.
:::

## Node.js — Sharp (путь через сырой буфер)

Для модели Brettel или когда нужны точные результаты с корректным гамут-маппингом:
...
## Node.js — Sharp (автоматический выбор пути)

Используйте `isMatrixModel()`, чтобы автоматически выбрать оптимальный путь:
...
## WebGL — Uniform шейдера

Передайте матрицу симуляции как uniform во фрагментный шейдер:
...
## CSS-фильтр (приближённый)

Для быстрого прототипирования можно использовать CSS `filter` с SVG `feColorMatrix`. Это менее точно, но не требует JavaScript:
...
::: warning
Этот способ пропускает линеаризацию sRGB и гамут-маппинг — это приближение. Используйте Viénot для лучших результатов, так как её матрицы неотрицательны.
:::

## React — Split View с use-split-view

Создайте сравнение до/после CVD с помощью [`use-split-view`](https://github.com/NemeZZiZZ/use-split-view) — headless-хука для React с поддержкой масштабирования, панорамирования и сенсорного ввода.
...
`use-split-view` обрабатывает всё взаимодействие (перетаскивание указателем, сенсорный ввод, масштабирование и панорамирование трекпадом), а chromanopia отвечает за цветовую науку. Хук полностью headless — вся разметка и стили остаются за вами.
